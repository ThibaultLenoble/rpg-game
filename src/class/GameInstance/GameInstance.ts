...
export default class GameInstance {
  ...
  newGame() {
    this.buildMap();
    this.actualRoom = changeEvent(this.eventBuilder.build(this.eventBuilder.getEvent(2, 'main')), this.render);
  }
    ...
    switch (choice.action) {
      ...
      case "chest-heal":
        ...
        this.eventBuilder.build(
          this.eventBuilder.getEvent(2, 'main')
        ).outputContext = `Vous avez été soigné de ${heal} ❤️.`;
        this.actualRoom = changeEvent(
          this.eventBuilder.build(this.eventBuilder.getEvent(1, 'main')),
          this.render
        );
        break;
      case "chest-hp-plus":
        ...
        event = this.eventBuilder.build(this.eventBuilder.getEvent(2, 'main'));
      ...
      case "tacos":
        ...
        if (this.randomIntFromInterval(1, 2) == 1) {
          ...
          this.eventBuilder.build(
            this.eventBuilder.getEvent(1, 'main')
      ...
      case "chest-hit":
        ...
        if (this.player.currentLife <= 0) {
          ...
          const event = this.eventBuilder.build(this.eventBuilder.getEvent(1, 'main'));
      ...
      case "chest-earn-money":
        ...
        event = this.eventBuilder.build(this.eventBuilder.getEvent(1, 'main'));
